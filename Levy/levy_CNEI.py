...
from botorch.acquisition import PosteriorMean, AnalyticAcquisitionFunction, UpperConfidenceBound, AcquisitionFunction, ExpectedImprovement, LogNoisyExpectedImprovement
...
import random
from botorch.acquisition.analytic import _scaled_improvement, _ei_helper, _log_ei_helper
...
class LNEI(ExpectedImprovement):
    ...
    def __init__(
        ...
        # hyperparameters = {k: v for k, v in model.state_dict().items() if k in fantasy_model.state_dict()}
        # fantasy_model.load_state_dict(hyperparameters)
...
if __name__ == "__main__":
    ...
    import pandas as pd
    pd.DataFrame(BOD_table).T.to_excel("levy_cnei_bod.xlsx", index=False, engine='openpyxl')
    pd.DataFrame(simple_table).T.to_excel("levy_cnei_int.xlsx", index=False, engine='openpyxl')